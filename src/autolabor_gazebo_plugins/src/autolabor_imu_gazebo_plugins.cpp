...
#include <autolabor_gazebo_plugins/autolabor_imu_gazebo_plugins.h>
#include <iostream>
...
namespace autolabor_gazebo 
{
    ...
    void ImuPlugin::Load(gazebo::sensors::SensorPtr sensor_, sdf::ElementPtr sdf_) 
    {
        sdf = sdf_;
        ...
        connection = gazebo::event::Events::ConnectWorldUpdateBegin(
                boost::bind(&ImuPlugin::UpdateChild, this, _1));

        last_time = sensor->LastUpdateTime();
    }

    void ImuPlugin::UpdateChild(const gazebo::common::UpdateInfo &/*_info*/) 
    {
    ...
    double ImuPlugin::GuassianKernel(double mu, double sigma) 
    {
    ...
    bool ImuPlugin::LoadParameters() 
    {
        ...
        if (sdf->HasElement("xyzOffset")) 
        {
            ...
            ROS_INFO_STREAM(
                    "<xyzOffset> set to: " << offset.Pos()[0] << ' ' << offset.Pos()[1] << ' ' << offset.Pos()[2]);
        } 
        else 
        {
        ...
        else 
        {
            ...
            ROS_WARN_STREAM(
                    "missing <rpyOffset>, set to default: " << offset.Rot().Roll() << ' ' << offset.Rot().Pitch() << ' '
                                                            << offset.Rot().Yaw());
        }

        if (sdf->HasElement("enableOrientation")) 
        {
            enable_orientation = sdf->Get<bool>("enableOrientation");
            ROS_INFO_STREAM("<enableOrientation> set to: " << enable_orientation);
        } 
        else 
        {
            enable_orientation = true;
            ROS_WARN_STREAM("missing <enableOrientation>, set to default: " << enable_orientation);
        }
