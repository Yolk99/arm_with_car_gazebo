...
namespace autolabor_gazebo 
{
    ...
    void FourWheelDiffDriver::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) 
    {
        ...
        if (this->publishWheelJointState_) 
        {
            ...
            //
            //
            //***************************这里进行了修改，joint_states改为arm/joint_states***************************
            //
            //
            //joint_state_publisher_ = gazebo_ros_->node()->advertise<sensor_msgs::JointState>("joint_states", 1000);
            joint_state_publisher_ = gazebo_ros_->node()->advertise<sensor_msgs::JointState>("arm/joint_states", 1000);
        ...
        ros::SubscribeOptions so =
                ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,boost::bind(&FourWheelDiffDriver::cmdVelCallback,this, _1),ros::VoidPtr(), &queue_);
    ...
    void FourWheelDiffDriver::UpdateChild() 
    {
        ...
        */
        ...
        if (seconds_since_last_update > update_period_) 
        {
            ...
            if (wheel_accel_ == 0 || (fabs(wheel_speed_[LEFT] - current_speed[LEFT]) < 0.01) || (fabs(wheel_speed_[RIGHT] - current_speed[RIGHT]) < 0.01)) 
            {
    ...
    void FourWheelDiffDriver::publishOdomTF() 
    {
        ...
        transform_broadcaster_->sendTransform(tf::StampedTransform(base_footprint_to_odom, ros::Time::now(), odometry_frame_, robot_base_frame_));
    }
    ...
    void FourWheelDiffDriver::publishWorldTF() 
    {
        ...
        transform_broadcaster_->sendTransform(tf::StampedTransform(base_footprint_to_world.inverse(), ros::Time::now(), robot_base_frame_, world_frame_));
    }
